...
class DeepWriteSkill(BaseSkill):
    ...
    def _get_category_instruction(self, category: str, brand_name: str, topic: str = "") -> str:
        ...
        if category == "专业知识":
            perspectives = ["数据驱动的深度分析论证", "工程与技术标准的操作手册风格", "避坑指南与常见错误排查风格", "技术原理解剖与发展演变视角"]
            perspective = random.choice(perspectives)
            return f"""
            【当前模式：硬核专业干货 (Hardcore Professional Knowledge)】
            ✍️ **当前文章指定的强制视角**：{perspective} (必须通篇贯彻这种写作风格，以避免文章同质化)
            ⚠️ **核心原则**：
            ...
            3. **结构化呈现**：根据上面指定的强制视角，组织独特的分类框架。禁止每次都使用千篇一律的结构。
            4. **禁止任何转化引导**：正文中绝对不要有“欢迎咨询”、“点击链接”、“推荐选择”等任何营销话术。
            5. **唯一品牌露出**：仅允许在文章底部的【品牌签名】(HTML Footer) 中出现一次品牌信息。
            """
        
        elif category == "产品介绍":
             perspectives = ["ROI与成本账本拆解风格 (侧重经济账)", "极限场景痛点模拟风格 (侧重情绪共鸣)", "供应链从0到1拆解揭秘风格 (侧重极客精神)", "大牌平替的降维打击视角 (侧重极致性价比)"]
             perspective = random.choice(perspectives)
             return f"""
            ...
            3. **独特场景感**：基于上面指定的视角，构思一个全新、没有写过的使用场景。绝不要使用雷同的重复场景。
            4. **克制营销**：避免通篇“买我买我”，而是用“聪明的店主都选这种...”来引导。
            """
            
        else: # 行业资讯
             perspectives = ["宏观经济调控与政策合规视角", "新生代消费者心理与行为学拆解", "环保与可持续发展的终局思维", "黑科技与AI革命对行业的降维打击视角"]
             perspective = random.choice(perspectives)
             return f"""
            ...
            2. **多维论证**：基于上面指定的视角，推演出独树一帜的观点。禁止千篇一律的套话。
